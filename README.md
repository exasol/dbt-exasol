...
<ul>
  ...
  <li><strong>timestamp_format</strong>: default: <code>YYYY-MM-DDTHH:MI:SS.FF6</code>. The adapter applies this value with <code>ALTER SESSION SET NLS_TIMESTAMP_FORMAT</code> when it creates each connection, so it affects model SQL, snapshots and seed imports—not only seeds. The <code>T</code> separator intentionally supports ISO-8601 seed files (see <a href="https://github.com/exasol/dbt-exasol/issues/35">#35</a>), but differs from Exasol's server default, <code>YYYY-MM-DD HH24:MI:SS.FF6</code>. Set this parameter to the server value when SQL or data must have the same timestamp parsing behavior in dbt and other clients.</li>
